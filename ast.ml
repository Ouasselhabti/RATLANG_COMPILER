...
type expression =
  ...
  (* Pointeur null *)
  | Null
  (*conditionnelle sous la forme d’un op ́erateur ternaire*)
  | Tenaire of expression * expression * expression
...
and instruction =
  ...
  (*Conditionnelle représenté par la condition , le bloc then et sans le bloc else*)
  | ConditionnelleSimple of expression * bloc
  ...
  | Loop of bloc
  (*les boucles étiquettées*)
  | LabeledLoop of string * bloc
  (*Instruction break*)
  | Break
  (*Instruction break étiquettée*)
  | LabeledBreak of string
  | Continue
  | LabeledContinue of string 
...
let rec chercherAffectable a = match a with 
  | Valeur a2 -> chercherAffectable a2
  | Ident n -> n
end
...
struct
  ...
  and instruction =
    ...
    (*les boucles Loop à la Rust*)
    | Loop of bloc
    | LabeledLoop of Tds.info_ast * bloc
    | Break
    | LabeledBreak of Tds.info_ast
    | Continue
    | LabeledContinue of Tds.info_ast 
...
type bloc = instruction list
 and instruction =
  ...
  | Empty (* les nœuds ayant disparus: Const *)
  (*le bloc else optionnel dans la conditionnelle*)
  | Optionelle of expression * bloc
  (*les boucles Loop à la Rust*)
  | Loop of bloc
  | LabeledLoop of Tds.info_ast * bloc
  | Break
