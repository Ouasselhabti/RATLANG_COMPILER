...
(* Produit le code correspondant à l'instruction. L'execution de ce code 
   ...
   let rec analyse_code_expression e = 
    ...
    | AstType.Unaire (unaire,e) -> (match unaire with 
...
let rec analyse_code_instruction i = 
  ...
  | AstType.TantQue(c, b) ->
      ...
      "JUMP "^etiq^"\n"^etiq2^"\n"             
  | Empty -> ""
  | AstType.ConditionnelleSimple (cond, b) -> 
    let ne = analyse_code_expression cond in 
    let nb1, pop_taille_if = analyse_code_bloc b1 in ""
  |AstType.Loop(b) -> ""
  |AstType.LabeledLoop(s,b) -> ""
  |AstType.Continue -> ""
  |AstType.LabeledContinue(s) ->""
  |AstTtpe.Break ->""
  |AstType.LabeledBreak(s) ->""
